...
class ScorePlot(QWidget):
    ...
    def load_user(self, user_data: UserData):
    ...
    def update_user_items(self, x_range: tuple[float, float]):
        ...
        user_pitches = self.user_data.pitch_data.read(x_range[0], x_range[1], clean=True) if self.user_data else []
        user_notes = self.user_data.note_data.read(
            start_time=x_range[0], 
            end_time=x_range[1], 
            clean=True
        ) if self.user_data else []
        ...
        self.user_pitches.setData(x=xs, y=ys)

        if self.user_data.note_data is None:
            self.user_notes.setOpts(x=[], width=[], y0=[], height=[])
            return
    ...
    def update_midi_items(self, x_range: tuple[float, float]):
        ...
        midi_notes = self.midi_data.note_data.read(x_range[0], x_range[1]) if self.midi_data else []
...
class RunScorePlot:
    def __init__(self, user_data: UserData=None, midi_data: MidiData=None, alignment: Alignment=None, app=None):
        import sys
        ...
        if midi_data is not None:
            self.vis.load_midi(midi_data)
        if user_data is not None:
            self.vis.load_user(user_data)
