...
class Slider(QWidget):
    ...
    # --- RANGE HANDLING ---
    def update_slider_range(self, midi_data=None, user_data=None):
        """Update the slider range based on max(MIDI.length, audio.length)"""
        m0, m1, u1 = 0, 0, 0
        if midi_data:
            m0, m1 = midi_data.note_data.get_bounds()
        if user_data and user_data.audio_data:
            u1 = user_data.audio_data.get_length()

        x0 = m0
        x1 = max(m1, u1+m0)
        self._update_slider_range(x0, x1)

    def _update_slider_range(self, start_time: float, end_time: float):
